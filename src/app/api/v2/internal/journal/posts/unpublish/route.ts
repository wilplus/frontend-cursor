...
/**
 * POST /api/v2/internal/journal/posts/unpublish
 *
 * BFF passthrough for the Journal CMS (flip a post back to draft (keeps its date)).
 *
...
export async function POST(req: NextRequest) {
  ...
  try {
    upstream = await fetch(`${backend}/v2/internal/journal/posts/unpublish`, {
      method: "POST",
    ...
    console.error("POST /api/v2/internal/journal/posts/unpublish — fetch failed:", err);
    return NextResponse.json(

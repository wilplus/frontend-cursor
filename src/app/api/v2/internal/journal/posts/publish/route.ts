...
/**
 * POST /api/v2/internal/journal/posts/publish
 *
 * BFF passthrough for the Journal CMS (flip a post to published).
 *
...
export async function POST(req: NextRequest) {
  ...
  try {
    upstream = await fetch(`${backend}/v2/internal/journal/posts/publish`, {
      method: "POST",
    ...
    console.error("POST /api/v2/internal/journal/posts/publish — fetch failed:", err);
    return NextResponse.json(

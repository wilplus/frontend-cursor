import { internalJournalPassthrough } from "@/app/api/v2/internal/journal/_shared";

/**
 * POST /api/v2/internal/journal/community/delete
 *
 * BFF passthrough for the Community Content Studio: removal of one community draft.
 *
 * Community drafts are NOT journal posts: no slug, no status, no public
 * route ever serves them. This proxy only relays (see _shared.ts for the
 * password-gating and error-relay behavior common to every internal tool).
 */
export const runtime = "nodejs";
export const POST = internalJournalPassthrough("/v2/internal/journal/community/delete");

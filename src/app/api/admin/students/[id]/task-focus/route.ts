...
export async function GET(
  ...
  if (!backend) {
    return NextResponse.json({ task_focus: [] });
  }
  const { id } = await params;
  let res: Response;
  try {
    res = await fetch(`${backend}/v2/admin/students/${id}/task-focus`, {
  ...
  const task_focus = Array.isArray(data.task_focus) ? data.task_focus : [];
  return NextResponse.json({ task_focus });
}

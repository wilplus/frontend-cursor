 ...
 * 
 * NOTE: This is the updated solo-relevant version.
 * The backend should use this pool when generating post-questions.
 */
...
export const POST_QUESTIONS_POOL: PostQuestionSet[] = [
  // Awareness & Attention
  {
    id: 1,
    name: "Awareness & Attention",
    q1: { text: "How aware were you of your voice?", type: "scale" },
    q2: { text: "Did you notice yourself before using filler words?", type: "binary" },
 ...
 * Can be enhanced with user history tracking
 */
export function selectQuestionSet(
  ...
  const available = POST_QUESTIONS_POOL.filter((set) => !usedIds.has(set.id));
  ...
  return POST_QUESTIONS_POOL[Math.floor(Math.random() * POST_QUESTIONS_POOL.length)];
}

...
import { adminApi, type CharismaSnippet } from "@/lib/api/admin-client";
...
function snippetStartSec(s: CharismaSnippet): number {
  return secondsWithMsFallback(s.start_sec ?? s.startSec, s.start_ms);
}

function snippetEndSec(s: CharismaSnippet): number {
  return secondsWithMsFallback(s.end_sec ?? s.endSec, s.end_ms);
}

function snippetDurationSec(s: CharismaSnippet): number {
  return secondsWithMsFallback(s.duration_sec ?? s.durationSec, s.duration_ms);
}

function formatTimingRange(s: CharismaSnippet): string {
...
export default function CharismaDojoWorkspace({ showHeader = true }: { showHeader?: boolean }) {
  ...
  const [snippets, setSnippets] = useState<CharismaSnippet[]>([]);
  ...
  const loadSnippets = useCallback(async (options?: { quiet?: boolean }) => {
    ...
    try {
      const response = await adminApi.listCharismaSnippets({
  ...
  useEffect(() => {
    ...
    async function loadSettings() {
      try {
        const res = await adminApi.getCharismaSnippetSettings();
  ...
  const toggleAutoExtract = useCallback(async () => {
    ...
    try {
      const res = await adminApi.updateCharismaSnippetSettings(next);
  ...
  const labelSnippet = useCallback(
    async (snippet: CharismaSnippet, label: "charisma" | "no_charisma") => {
      const notes = (notesBySnippetId[snippet.id] || "").trim();
      // Notes are REQUIRED when label = "charisma", optional for "no_charisma"
      if (label === "charisma" && !notes) {
        toast.error("Notes are required when labeling as Charisma — describe what makes this charismatic.");
        return;
      }
      ...
      try {
        await adminApi.labelCharismaSnippet(snippet.id, {
          label,
          notes: notes || null,
        });
        const reviewLabel: RecentReview["label"] = label === "charisma" ? "Charisma" : "No Charisma";
  ...
  const undoLabel = useCallback(
    async (snippet: CharismaSnippet) => {
      ...
      try {
        await adminApi.unlabelCharismaSnippet(snippet.id);
  ...
  const toggleSkip = useCallback(
    async (snippet: CharismaSnippet) => {
      ...
      try {
        if (willSkip) {
          await adminApi.queueSkipCharismaSnippet(snippet.id);
        } else {
          await adminApi.queueUnskipCharismaSnippet(snippet.id);
        }
  ...
  const regenerateForRecording = useCallback(
    async (recordingId: string) => {
      ...
      try {
        const res = await adminApi.generateCharismaSnippets(trimmed, {
    ...
    <div className="space-y-5 animate-fade-in-short">
      {showHeader ? (
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold">Charisma Pipeline</h1>
            <p className="text-sm text-muted-foreground">
              Label charisma / no charisma for ≤5s student and uploaded clips.
            </p>
          </div>
          <AutoExtractToggle
...
interface SnippetCardProps {
  snippet: CharismaSnippet;
  labeling: boolean;
  notes: string;
  onNotesChange: (value: string) => void;
  onLabel: (label: "charisma" | "no_charisma") => void;
  onUndo: () => void;
  onSkipToggle: () => void;
  onRegenerate: () => void;
...
function SnippetAudio({
  ...
  useEffect(() => {
    ...
    const fetchFreshUrl = async () => {
      try {
        ...
        const { playback_url: freshUrl } = await adminApi.getCharismaSnippetPlaybackUrl(snippet.id);
        ...
        if (freshUrl) {
          ...
          console.warn(`[CharismaAudio] Fresh URL endpoint returned empty for snippet ${snippet.id}, using original`);
        ...
        console.warn(`[CharismaAudio] Failed to fetch fresh URL for snippet ${snippet.id}, falling back to original`, err);
        setPlaybackUrl(snippet.audio_url);
      }
    };
  ...
  // per-snippet ≤5s mp3 at `charisma_snippets/<recording_id>/<snippet_id>.mp3`.
  ...
  return (
    <audio
      ...
      src={playbackUrl}
      onError={() => {
        console.warn(`[CharismaAudio] Failed to load audio for snippet ${snippet.id}`, {
  ...
  return (
    <>
      <div className="flex items-center justify-between border-b px-5 py-4">
        <div>
          <h2 className="text-3xl font-semibold tracking-tight">Charisma Label</h2>
      ...
      <div className="space-y-5 px-5 pb-5">
        <div className="rounded-xl border bg-muted/20 p-5">
          ...
          <textarea
            ...
            placeholder="Notes — required when labeling as Charisma (what makes this charismatic?)"
            className="mt-3 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
          />
        </div>

        <p className="text-center text-2xl font-semibold">Is this snippet charisma or no charisma?</p>

        <div className="flex flex-wrap items-center justify-center gap-3">
          <Button
            className="h-14 min-w-[180px] text-xl bg-emerald-600 text-white shadow-md hover:bg-emerald-700 focus-visible:ring-emerald-500"
            onClick={() => onLabel("charisma")}
            disabled={labeling}
          >
          ...
          <Button
            className="h-14 min-w-[180px] text-xl bg-red-600 text-white shadow-md hover:bg-red-700 focus-visible:ring-red-500"
            onClick={() => onLabel("no_charisma")}
            disabled={labeling}
          >
    ...
    <div className="flex flex-col gap-3 px-5 py-4">
          ...
          {snippet.coach_label ? (
            <span
              className={`rounded px-2 py-0.5 font-semibold ${
                snippet.coach_label === "charisma"
                  ? "bg-emerald-100 text-emerald-700"
                  : "bg-red-100 text-red-700"
              }`}
            >
              {snippet.coach_label === "charisma" ? "Charisma" : "No charisma"}
            </span>
          ) : null}
          {snippet.queue_skipped ? (
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          value={notes}
          onChange={(event) => onNotesChange(event.target.value)}
          placeholder="Notes (required for Charisma)"

...
type SessionItem = { id: string; created_at?: string; report_preview?: { report_text_preview?: string } };
...
export default function HomeworkReportsModal({ open, onOpenChange }: HomeworkReportsModalProps) {
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <h2 id="homework-reports-modal-title" className="text-lg font-semibold">
              {selectedSessionId ? "Report" : "Past reports"}
            </h2>
          </div>
          <Button
            type="button"
                  ...
                  {sessions.map((s) => (
                    <li key={s.id}>
                      <button
                        ...
                        className="w-full rounded-lg border border-border bg-muted/30 px-4 py-3 text-left hover:bg-muted/50 transition-colors"
                      >
                        <span className="font-medium text-foreground">{dateLabel(s)}</span>

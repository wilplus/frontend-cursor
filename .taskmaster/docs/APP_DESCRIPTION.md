...
| 5    | Report         | Views report and score      | `completed`        | **Entered only after POST post-answers succeeds.** Use the **post-answers response body** (report_text, performance_score_end) to render step 5. **Do not** call GET status to show the report: GET status does **not** return completed sessions. **Step 5 UI (order):** (1) Final recording player, (2) **Progress over sessions** line chart (from GET report; optional `performance_history`: last 5 sessions’ date + score; else single point from current overall), (3) report text, (4) "Start new homework" button. **Navbar** visible on step 0 and step 5 only; **progress bar** on steps 1–4 only (none on step 5). "Start new homework" resets session to step 0 without logging out. See FRONTEND-FLOW-AND-CHANGES.md. |
...
- **Warm-up text:** `warmUpText = response.warm_up_task?.text ?? response.session?.warm_up_task_text ?? ""`. When this is **empty or null**, the frontend uses the **default warm-up question**: “How was your day so far?” (see `docs/handoffs/backend-summary-default-warmup.md`). Backend can omit warm-up; frontend never blocks step 1 for missing warm-up.
...
- **Recording_1 duration:** Frontend enforces **min 30s** for the warm-up (first) recording; UI shows "min. 30s", Stop disabled until elapsed ≥ 30s; validation before upload. Backend should accept recording_1 with min ≥ 30s (see backend prompt if it previously required 60s).
...
- **Report:** Report is generated by the backend when **post-answers** are submitted (no separate POST /report). **Step 5 content:** On mount, frontend calls **GET report** (sessionId) for audio_url, report_text, scores, and optional **performance_history** (array of `{ date, score }`, last 5 sessions, oldest first). If `performance_history` is absent, the "Progress over sessions" chart shows one point (current overall score). Tooltip on chart dot shows date and overall score. **Step 5 is entered from POST post-answers response** (report_text, performance_score_end); GET status does not return completed sessions.
...
- **Wheel (dartboard):** Strength and pace are **client-side only** (useRealtimeStrengthPace, AnalyserNode). **100 ms** update interval. **Dual EMA** (fast + slow) for trend-based feedback: strength 0.7 slow + 0.3 fast with return-to-center damping; pace 0.6 slow + 0.4 fast. Quiet side 22% less sensitive. Ball lerp 0.12. Start the interval only after `ctx.resume()` so the analyser gets real audio. See FRONTEND-FLOW-AND-CHANGES.md.
- **Mic permission:** Requested only on user action. Permission is started on **pointer down** on "Start Recording" so the browser treats it as the same user gesture (helps Safari/iOS). On NotAllowedError, show clear copy to allow mic in address bar / site settings (see FRONTEND-FLOW-AND-CHANGES.md).
...
| Area | Path / role |
|------|-------------|
| **HomeworkFlowCard** | `src/components/homework/HomeworkFlowCard.tsx` — GET status / POST start, applyStatusToState, deriveStepFromStatus (five statuses only), refreshStatus (when no active session → full reset to step 0). Step 5: GET report on mount; layout (1) final recording, (2) ProgressOverSessionsChart, (3) report text, (4) "Start new homework" button; **no progress bar** on step 5 (navbar only at top). Default warm-up “How was your day so far?” when backend sends none. Abandon 404 → success, step 0. |
| **ProgressOverSessionsChart** | `src/components/homework/ProgressOverSessionsChart.tsx` — Step 5: line chart "Progress over sessions" (S1–S5, orange line, tooltip date + overall score). Data from report.performance_history or single point from scores.overall. |
| **AnswerMetricQuestionsScreen** | `src/components/homework/AnswerMetricQuestionsScreen.tsx` — Step 2: task block + metric inputs + submit. Same style as post-questions (rounded-lg, space-y-8, full-width Continue). |
| **PostQuestionsStepScreen** | `src/components/homework/PostQuestionsStepScreen.tsx` — Step 4: reflective questions; same style as metric questions (placeholder "Describe your answer…", mt-8 on button). |
...
| **middleware** | `middleware.ts` — Supabase session (getUser); strip auth params from URL; redirect to login for protected and admin routes when no session; cookie options Secure/SameSite/HttpOnly. |
| **(protected) layout** | `src/app/(protected)/layout.tsx` — Server-side getUser(); redirect to login if no user (safeguard so shared links never show another user's session). |

---

## 7.5 Auth and session (protected routes, shared links, persistence)

- **Protected routes:** `/dashboard`, `/profile`, `/recordings`, `/change-password`, and `/admin` require a valid Supabase session. **Middleware** (`middleware.ts`) runs on every request: calls `getUser()`; if no session, redirects to `/login?redirectTo=<path>`. **Protected layout** (`src/app/(protected)/layout.tsx`) runs for all routes under `(protected)` and redirects to login if `getUser()` returns no user (server-side safeguard).
- **Shared links:** If someone opens a link like `app.willonski.com/dashboard` in another browser or device, they have no session cookies → middleware and/or layout redirect them to login. **No auth in URL:** Middleware strips query params `access_token`, `refresh_token`, `token`, `api_key`, `supabase_key` from any request and redirects to the same path without them, so sharing a link never passes credentials.
- **Persistence (“browser remembers”):** Session is stored in **cookies** (Supabase SSR). Cookie options in middleware: `Secure` in production, `SameSite: lax`, `HttpOnly`, `path: "/"`. Users stay logged in across tabs and restarts until they click Logout.
- **Logout:** Dashboard header (and admin) call `supabase.auth.signOut()` then redirect to `/login`. No token or session in URL at any time.
...
- **Shared link shows wrong user** — Must not happen: session is in cookies only; middleware and (protected) layout validate on every load; auth params stripped from URL. If it does, verify middleware runs for the path and (protected) layout runs for dashboard/profile/recordings.

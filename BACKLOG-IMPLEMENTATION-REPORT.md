...
### 3/4 — AI image generate/refine/regenerate for posts — CONFIRMED DONE
`CoverImageStudio` in the CMS has the full loop: notes box to steer ("darker, no hands…"), Draw/Regenerate, refine-selected-attempt, attempt history with restore. Deliberately not rebuilt here.

> **Correction (2026-07-30).** This item originally carried a ⚠️ claiming the backend routes `/v2/internal/journal/image/{generate,list,select,delete}` did not exist and that the studio would 404 in production. **That was wrong** — a truncated grep. The handlers are on backend `main` (`routes/journal.py:609`, shipped in `c383625` / #284) and the FE proxies are on frontend `main`. Nothing to merge; the cover studio works.
...
**Built (your approved shape):** web push (VAPID) strictly **opt-in, all defaults OFF**, eight slots — daily morning, daily evening, weekly, plus the checkout ladder (monthly, quarterly, yearly, fiveyear, tenyear) — toggles in the panel's data/settings area; service worker push + notificationclick handlers; new `life_push_subscriptions` / `life_reminder_settings` / `life_reminder_log` tables (idempotent per user/slot/day); delivery isolated in `services/life_reminders.py` (the ONLY module allowed to touch pywebpush); internal cron endpoint + `Dockerfile.life-reminders-cron` (morning 05:00, evening 19:00, weekly Sun 17:00 UTC — adjust to your tz). **NoNudgesTests rewritten, not deleted:** the fence now records your 2026-07-30 decision and holds the amendment at exactly this width — generation paths still contain zero delivery code, defaults-off is itself a test.

> **Correction (2026-07-30).** This item originally said "three slots" and "Monthly/quarterly/yearly/5y/10y checkouts: deferred per your choice." **Both were stale** — the ladder was added before ship. The FE renders all eight toggles (`SLOTS` + `CHECKOUT_SLOTS` in `ReminderSettings.tsx`), the migration carries all eight `*_enabled` columns, and `LIFE_REMINDER_SLOT` accepts all eight values. Note the cron schedules in `Dockerfile.life-reminders-cron` cover the first three; the ladder slots need their own cron services when you want them live.

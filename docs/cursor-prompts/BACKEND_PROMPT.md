...
- **Key modules:** `db.py`, `openai_service.py`, `email_service.py`. Migrations: `v2_all_in_one.sql` + coaching migrations.
...
1. **Locate where `performance_score_2` is set:**
   - Search for `performance_score_2` in the codebase (e.g. `services/metrics_v2.py`, `services/recording_2_job.py`, or the job that runs after recording-2 upload).
...
3. **Background job / async path:**
   - If recording-2 is processed in a background job (e.g. `recording_2_job.py`), confirm the job writes the computed score to the session/recording row and that the POST recording-2 response or GET status later returns it.
   - If the job is slow, ensure the frontend can receive the score via GET report or GET status once the job completes (no need to change flow; just ensure DB is updated correctly).
...
**Expected behavior:** Step 5 report section shows something like: *"12 filler words detected. Time in good vocal range: 45 seconds (75% of recording)."* No extra narrative unless product explicitly asks for it later.
...
**Fix:** Add comments in the relevant services (e.g. `db.py`, `openai_service.py`) and optionally a small doc (e.g. `docs/CONTEXT_FIELDS.md`) so future changes don’t confuse these fields.
...
1. **Locate the email template:**
   - Find where the assignment email is built and sent (e.g. `email_service.send_assignment_email` or equivalent in `services/email_service.py`).
...
2. **API contract:**
   - **POST /v2/admin/students/<id>/send-assignment**
     ...
     - Backend stores `video_url` (e.g. in assignment row or a new column). When creating or updating the “current assignment” for that student, persist this URL so it can be returned with GET session/status or equivalent.
   - **GET /v2/homework/session/status** (or the endpoint that returns current session/assignment for the student)
     - Include `tutor_video_url` (or `video_url`) in the response when the backend has one for the current student/assignment. Frontend will show it on step 0 or step 5.
...
4. **Migrations:**
   - Add the new column(s) (e.g. `tutor_video_url` on the table that backs “current assignment” or session). Run migration in dev and production.

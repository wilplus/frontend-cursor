-- Fix question_type check constraint
-- The constraint might have incorrect allowed values or be missing
...
-- Step 3: Check what constraints exist
SELECT 
...
-- Step 4: Drop existing check constraint if it exists (with different name variations)
DO $$
DECLARE
    r RECORD;
BEGIN
...
-- Step 5: Verify all data is now valid before adding constraint
SELECT 
    question_type,
    COUNT(*) as count
FROM public.post_recording_questions
...
GROUP BY question_type;
-- If the above query returns rows, there's still invalid data that needs manual fixing
...
-- Step 8: Final verification - all question types should be valid
SELECT 
    question_type,
    COUNT(*) as count
FROM public.post_recording_questions
